...
const Header = () => {
  ...
  const [showPlayListPopup, setShowPlayListPopup] = useState(false); // 플레이리스트 팝업 표시 상태
  ...
  return (
    <>
      <header id='header' role='banner'>
        ...
        <button className="show-playlist-button" onClick={() => setShowPlayListPopup(true)}>
          Show Play List
        </button>
      </header>
      {showPlayListPopup && (
        <div className="playlist-popup">
          <button className="close-button" onClick={() => setShowPlayListPopup(false)}>Close</button>
          <h3><IoMusicalNotes /> Play list</h3>
          <ul>
            {[...Array(playlistCount).keys()].map(i => {
              const playlistKey = `playlist${i + 1}`;
